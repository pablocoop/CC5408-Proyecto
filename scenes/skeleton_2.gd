...
@onready var pivot_y: Node2D = $Pivot_y
@onready var ray_cast_2d_y: RayCast2D = $Pivot_y/RayCast2D_y

@onready var progress_bar: ProgressBar = $HealthBar/MarginContainer/ProgressBar
...
func _physics_process(delta: float) -> void:
	...
	velocity.y = move_toward(velocity.y, max_speed * pivot_y.scale.y, acceleration * delta)
	playback.travel("movement")
	move_and_slide()
	
	if ray_cast_2d.is_colliding():
		pivot.scale.x *= -1
	if ray_cast_2d_y.is_colliding():
		pivot_y.scale.y *= -1
