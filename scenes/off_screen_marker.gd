extends Node2D
@onready var sprite_pointer: Sprite2D = $sprite_pointer
...
func _process(delta: float) -> void:
	...
	_set_marker_position(Rect2(top_left, size))
	_set_market_rotation()
	
	
func _set_marker_position(bounds: Rect2):
	if target_position == null:
		sprite_pointer.global_position.x = clamp(global_position.x, bounds.position.x, bounds.end.x)
		sprite_pointer.global_position.y = clamp(global_position.y, bounds.position.y, bounds.end.y)
	else:
		...
		sprite_pointer.global_position = Vector2(length, 0).rotated(displacement.angle()) + target_position
...
func _set_market_rotation():
	var angle = (global_position - sprite_pointer.global_position).angle()
	sprite_pointer.global_rotation = angle
